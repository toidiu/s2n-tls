...
#define KTLS_enable() \
    if (ktls_enable_send) EXPECT_SUCCESS(s2n_config_ktls_enable(config));

#define KTLS_enable_check(conn) \
    if (ktls_enable_send) EXPECT_TRUE(conn->ktls_enabled_send_io);
...
static void ch_handler(int sig) { return; }
